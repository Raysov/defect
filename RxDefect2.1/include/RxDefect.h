...
typedef struct _tagRxDetInfo_{
	...
	unsigned int   uLowTErodeNum; // 低阈值分割结果腐蚀次数，范围0-10，默认值设为0，一般情况下不用调节，当出现缺陷和非缺陷粘连的情况，可以调节此参数，使缺陷和非缺陷断开
	unsigned int   uLowTDilateNum;// 低阈值分割结果膨胀次数，范围0-10，默认值设为0，一般情况下不用调节，当检测海绵状缺陷时可以将其调节为1，使检测出来的缺陷完整，具有连通性
	...
	unsigned int   uMaxArea;      // 去除伪缺陷，当缺陷的面积大于uMaxArea时，不对缺陷进行标注；取值设置为100-20000，默认值为8000，用于去除狭长的边缘伪缺陷；
